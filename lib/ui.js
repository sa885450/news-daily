...
function generateHTMLReport(summary, newsData, keywordStats = {}) {
    ...
    // 🟢 修正 2：分類按鈕使用 data-filter 屬性，避免文字比對錯誤
    ...
    if (sortedKeywords.length > 0) {
        const keywordTags = sortedKeywords.map(([key, count]) => {
            ...
            if (count >= 10) {
                colorClass = "bg-red-600 text-white border-red-600 shadow-md shadow-red-200 animate-pulse";
    ...
    <head>
        ...
        <script>
            ...
            function filterCategory(targetCat) {
                ...
                // 更新按鈕狀態
                btns.forEach(btn => {
                    // 使用 dataset.filter 比對，而非 innerText
                ...
                cards.forEach(card => {
                    ...
                    const cardCat = card.dataset.category; // 取得卡片分類
                    
                    if (targetCat === '全部' || cardCat === targetCat) {
                        card.classList.remove('hidden');
                        // 使用 requestAnimationFrame 確保動畫重播
