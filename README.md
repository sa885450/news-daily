...
## 📝 更新日誌 (Changelog)
### v8.5.0 - 視覺進化：行情掛件與佈局重構 (2026-03-05)
- **📟 市場行情掛件**: 前端介面新增實時行情橫幅，動態顯示 BTC、ETH、台股指數與美股指數，支援紅漲綠跌視覺標註。
- **🏗️ 佈局邏輯重構**: 根據使用者操作習慣，將「市場勢力地圖 (Knowledge Graph)」移動至「7日熱詞」下方，提升數據分析的連貫性。
- **📈 數據深度整合**: 擴展 `mcp.js` 行情指標，並將行情數據完整序列化至 `data.json`，實現前後端數據完全連動。
- **📦 版本同步**: 更新 `package.json` 版本號至 8.5.0。
