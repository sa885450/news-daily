...
│   └── report_*.html  #    歷史報表存檔
├── tests/             # 🧪 測試與驗證
│   ├── utils.test.js      #    單元測試 (Jest)
│   ├── test_discord.js    #    手動驗證腳本
│   └── test_ai_failure.js #    AI 失敗模擬
└── lib/               # 🧠 核心邏輯庫
    ├── config.js      #    設定檔載入器
    ├── utils.js       #    通用工具 (Winston Log, Discord)
    ├── db.js          #    資料庫操作
    ├── crawler.js     #    爬蟲與 API 抓取 (p-limit 並發)
    ├── ai.js          #    Gemini AI 分析 (Retry & Fallback)
    ├── ui.js          #    HTML 生成 (EJS Render)
    └── git.js         #    Git 自動化部署
```
