...
## 📝 更新日誌 (Changelog)
### v8.4.0 - 數據連動：MCP 行情數據整合 (2026-03-05)
- **📊 實時行情注入**: 整合 CoinGecko API，在 AI 分析流程中引入比特幣 (BTC) 與 乙太幣 (ETH) 的實時報價與漲跌幅。
- **🧠 數據對齊分析**: 優化 AI Prompt，使 AI 具備根據真實市場走勢校對新聞情緒的能力，產出更具數據支撐的金融摘要。
- **🔌 模組化連接器**: 新增 `src/lib/mcp.js` 模組，預留 Yahoo Finance MCP 接口，建立專案與外部金融數據源的標準橋樑。
- **📦 版本同步**: 更新 `package.json` 版本號至 8.4.0。
