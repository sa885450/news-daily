...
## 📝 更新日誌 (Changelog)
### v8.6.0 - 智慧架構：分層監控與實時預警 (2026-03-05)
- **📡 智慧監控模組 (`monitor.js`)**: 新增輕量級監控腳本，專門偵測市場價格異常（如 1h 跌幅 > 3%），支援高頻率異動報警。
- **🚨 雙頻道預警支援**: 在 `.env` 加入 `DISCORD_ALERT_WEBHOOK_URL`，實現「日報摘要」與「緊急預警」頻道分流。
- **🔄 前端熱同步**: `app.js` 實現了瀏覽器端 2 分鐘一次的實時行情更新，解決 2 小時後端頻率的時效性缺口。
- **📦 版本同步**: 更新 `package.json` 版本號至 8.6.0。
