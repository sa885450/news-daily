...
## 📝 更新日誌 (Changelog)
### v8.7.0 - 全方位監視：特定標的與多資產監控 (2026-03-05)
- **🎯 特定標的追蹤**: 針對使用者需求，將 **台積電 (2330.TW)**、**元大台灣50 (0050.TW)** 與 **凱基台灣Top50 (009816.TW)** 正式納入監控體系。
- **🔄 監控架構重構**: `monitor.js` 由單一標的監控升級為循環異動偵測系統，支援同時監視多種加密貨幣與台/美股標的。
- **📊 狀態精準記錄**: 升級 `monitor_state.json` 結構，精確記錄每個監控標的的歷史價位以計算即時波動。
- **📦 版本同步**: 更新 `package.json` 版本號至 8.7.0。
