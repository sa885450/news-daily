...
## 📝 更新日誌 (Changelog)
### v9.0.0 - 實戰進化：量化指標與 AI 策略聯動 (2026-03-05)
- **📊 量化技術指標**: 新增 `indicators.js` 模組，自動計算 14 日 RSI、MA5 與 MA20 均線，精確掌握市場強弱。
- **🧠 AI 策略建議**: 升級 AI 大腦，使其能結合「新聞情緒」與「量化指標」，在分析報告中提供具體的操作策略與進出場建議。
- **💎 進場機會預警**: `MarketMonitor` 衛星升級。現在不只監控跌幅，還會偵測「超賣 (RSI < 20)」信號，並發送專屬的「進場機會」綠色圖卡。
- **📦 版本同步**: 更新 `package.json` 版本號至 9.0.0。
