...
## 📝 更新日誌 (Changelog)
### v9.1.0 - 戰術升級：具體執行建議與賣點預警 (2026-03-05)
- **🎯 戰術執行模組**: AI 分析報告新增 `tactical_advice` 區塊，提供明確的「行動建議 (Action)」、「信心指數 (Confidence)」與「建議倉位 (Position Size)」，解決「怎麼下、下多少」的問題。
- **📦 賣點/減碼預警**: `MarketMonitor` 新增超買偵測邏輯。當 RSI > 75 時，發送「黃色減碼圖卡」，提醒投資人注意回測風險。
- **🐛 核心修復**: 修正 `index.js` 中錯誤的模組導入路徑，恢復 NewBot 服務的正常運作。
- **📦 版本同步**: 更新 `package.json` 版本號至 9.1.0。
