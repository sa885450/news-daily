...
## 📝 更新日誌 (Changelog)
### v8.3.0 - 專業重構：回歸中立金融分析 (2026-03-05)
- **🧹 移除特定社群標籤**: 徹底移除「8zz」、「巴逆逆」等特定社群指標的偵測與 Prompt 邏輯，確保報告內容的中立性與專業度。
- **⚖️ 強化客觀分析**: 優化 AI Prompt 結構，使其專注於「重大事件」與「實體關聯」，不再受特定個人言論干擾。
- **📦 版本同步**: 更新 `package.json` 版本號至 8.3.0。
